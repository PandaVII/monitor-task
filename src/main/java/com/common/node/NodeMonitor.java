...
public class NodeMonitor {
	 ...
	 */ 
	   ...
	   */ 
	  public Node getMonitorInfoBean() throws Exception { 
	    int kb = 1024 ; 
...
//	      cpuRatio = this .getCpuRatioForWindows(); 
	    ...
	    node.setFreePhysicalMemorySize(freePhysicalMemorySize); 
	    node.setMaxMemory(maxMemory); 
	    node.setOsName(osName); 
	    node.setTotalMemory(totalMemory); 
	    node.setTotalMemorySize(totalMemorySize); 
	    node.setTotalThread(totalThread); 
	    node.setUsedMemory(usedMemory); 
	    node.setCpuRatio(cpuRatio); 
	    return node; 
	  } 
	  ...
	  public  static  void main(String[] args) throws Exception { 
	    ...
	    Node monitorInfo = monitor.getMonitorInfoBean(); 
